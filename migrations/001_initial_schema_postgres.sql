-- Schelling Protocol v3.0 Schema - PostgreSQL Compatible
...
DROP TABLE IF EXISTS jury_verdicts CASCADE;
...
DROP TABLE IF EXISTS relay_blocks CASCADE;
...
DROP TABLE IF EXISTS declines CASCADE;
DROP TABLE IF EXISTS candidates CASCADE;
DROP TABLE IF EXISTS cluster_norms CASCADE;
...
DROP TABLE IF EXISTS vertical_descriptors CASCADE;
...
CREATE TABLE users (
  ...
  group_deadline           TIMESTAMPTZ,            -- ISO 8601
...
CREATE INDEX idx_traits_user     ON traits(user_token);
CREATE INDEX idx_traits_key      ON traits(key);
CREATE INDEX idx_traits_vis      ON traits(visibility);
...
CREATE INDEX idx_prefs_user      ON preferences(user_token);
CREATE INDEX idx_prefs_weight    ON preferences(weight);
...
CREATE INDEX idx_clusters_pop    ON clusters(population);

-- ─── Core: Cluster Norms ────────────────────────────────────────────
-- Emergent trait norms per cluster, updated as registrations accumulate.

CREATE TABLE cluster_norms (
  id                TEXT PRIMARY KEY,
...
CREATE INDEX idx_norms_cluster   ON cluster_norms(cluster_id);
...
CREATE INDEX idx_candidates_user_a   ON candidates(user_a_token);
CREATE INDEX idx_candidates_user_b   ON candidates(user_b_token);
CREATE INDEX idx_candidates_stages   ON candidates(stage_a, stage_b);
...
CREATE INDEX idx_declines_decliner   ON declines(decliner_token);
CREATE INDEX idx_declines_declined   ON declines(declined_token);
CREATE INDEX idx_declines_cluster    ON declines(cluster_id);
CREATE INDEX idx_declines_expires    ON declines(expires_at);
...
CREATE INDEX idx_contracts_proposer  ON contracts(proposed_by);
...
CREATE INDEX idx_deliverables_deliverer ON deliverables(deliverer_token);
...
CREATE INDEX idx_inquiries_candidate ON inquiries(candidate_id);
CREATE INDEX idx_inquiries_from      ON inquiries(from_token);
...
CREATE INDEX idx_messages_sender     ON messages(sender_token);
...
CREATE TABLE events (
  ...
  ack_deadline      TIMESTAMPTZ,                 -- ISO 8601
...
CREATE INDEX idx_events_emitter    ON events(emitter_token);
...
CREATE INDEX idx_subs_user       ON subscriptions(user_token);
...
CREATE INDEX idx_notif_sub       ON notifications(subscription_id);
...
CREATE INDEX idx_tools_owner     ON tools(owner_token);
...
CREATE INDEX idx_toolfb_tool     ON tool_feedback(tool_id);
...
CREATE INDEX idx_rep_identity    ON reputation_events(identity_id);
CREATE INDEX idx_rep_cluster     ON reputation_events(cluster_id);
CREATE INDEX idx_rep_type        ON reputation_events(event_type);
CREATE INDEX idx_rep_created     ON reputation_events(created_at);
...
CREATE INDEX idx_disputes_candidate  ON disputes(candidate_id);
CREATE INDEX idx_disputes_filed_by   ON disputes(filed_by);
CREATE INDEX idx_disputes_against    ON disputes(filed_against);
...
CREATE INDEX idx_jury_dispute    ON jury_assignments(dispute_id);
CREATE INDEX idx_jury_juror      ON jury_assignments(juror_token);
...
CREATE INDEX idx_verdicts_dispute ON jury_verdicts(dispute_id);
...
CREATE INDEX idx_verif_user      ON verifications(user_token);
CREATE INDEX idx_verif_trait     ON verifications(trait_key);
...
CREATE INDEX idx_enforcement_user ON enforcement_actions(user_token);
...
CREATE INDEX idx_pending_user     ON pending_actions(user_token);
CREATE INDEX idx_pending_type     ON pending_actions(action_type);
...
CREATE INDEX idx_idemp_created    ON idempotency_keys(created_at);

-- ─── Trigger for cleaning old idempotency keys ────────────────────
-- PostgreSQL version using a function and trigger
...
CREATE TRIGGER cleanup_idempotency_keys_trigger
