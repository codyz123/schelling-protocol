...
```bash
# 1. Is the API up?
curl -s https://schellingprotocol.com/schelling/describe | jq .protocol.version
# Should return "3.0"
...
curl -s -X POST https://schellingprotocol.com/schelling/clusters \
  -H 'Content-Type: application/json' \
...
```bash
# Wrong
curl https://schellingprotocol.com/schelling/search

# Right
curl -X POST https://schellingprotocol.com/schelling/search \
  -H 'Content-Type: application/json' \
...
- Ensure you're using `https://schellingprotocol.com` (not http)
...
- **API Docs:** https://schellingprotocol.com/docs (interactive Swagger UI)
